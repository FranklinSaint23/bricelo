  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="relative -mt-12 flex items-end gap-5 pb-6 border-b border-[var(--color-slate-200)]">
          <div className="relative h-24 w-24 rounded-[var(--radius-xl)] overflow-hidden bg-white border-4 border-white shadow-lg shrink-0">
            {store.logo_url
              ? <Image src={store.logo_url} alt={store.name} fill className="object-cover" sizes="96px" />
              : <div className="w-full h-full flex items-center justify-center bg-[var(--color-slate-100)]"><Store className="h-10 w-10 text-[var(--color-slate-400)]" /></div>
            }
          </div>
            ...
            <div className="flex items-center gap-4 mt-1 text-sm text-[var(--color-slate-500)]">
              {store.rating > 0 && (
                <span className="flex items-center gap-1">
                  <Star className="h-3.5 w-3.5 text-[var(--color-accent)] fill-[var(--color-accent)]" />
