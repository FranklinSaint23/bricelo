      ...
      <main className="flex-1 bg-[var(--color-slate-100)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-2xl font-bold text-[var(--color-navy-900)] mb-6">
            {t.myCart} {count > 0 && <span className="text-[var(--color-slate-500)] font-normal text-lg">({t.article(count)})</span>}
          </h1>
          ...
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[var(--radius-2xl)] border border-[var(--color-slate-200)]">
              ...
              <h2 className="text-lg font-semibold text-[var(--color-navy-900)]">{t.cartEmpty}</h2>
              <p className="text-sm text-[var(--color-slate-500)] mt-1 mb-6">{t.cartEmptySub}</p>
              <Button asChild size="lg">
                <Link href="/catalogue">{t.browseCatalog}</Link>
              </Button>
            </div>
          ) : (
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="lg:w-80 shrink-0">
                <div className="bg-white rounded-[var(--radius-xl)] border border-[var(--color-slate-200)] p-5 sticky top-20">
                  <h2 className="font-bold text-[var(--color-navy-900)] mb-4">{t.subtotal}</h2>
