  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {promoProducts.length > 0 && (
        <section>
          <SectionHeader title="Offres du moment" href="/catalogue" badge={{ label: 'PROMO', color: 'red' }} />
          <ProductGrid products={promoProducts} cols={4} />
        </section>
      )}

      {/* Vedettes — bannière sur mobile, grille sur desktop */}
      {featuredProducts.length > 0 && (
        <section>
          <SectionHeader title="Produits vedettes" href="/catalogue" badge={{ label: 'TOP', color: 'gold' }} />

          {/* Mobile : carousel/bannière */}
      ...
      {/* Tous les produits */}
      <section>
        <SectionHeader title="Tous les produits" href="/catalogue" />
        ...
        {allProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3 text-center bg-white rounded-2xl border border-dashed border-[var(--color-slate-300)]">
            <p className="text-base font-semibold text-[var(--color-navy-900)]">Aucun produit disponible</p>
            <p className="text-sm text-[var(--color-slate-500)] max-w-sm">
              Les vendeurs n'ont pas encore publié de produits. Revenez bientôt.
            </p>
          </div>
        ) : (
          <>
      ...
      <div className="rounded-xl bg-[var(--color-navy-900)] px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <p className="text-white font-bold">Vous vendez des produits au Cameroun ?</p>
          <p className="text-white/50 text-sm mt-0.5">Créez votre boutique et commencez à vendre en quelques minutes.</p>
        </div>
        <Link href="/register"
