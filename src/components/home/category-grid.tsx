  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[var(--color-navy-900)]">
            {fr ? 'Nos catégories' : 'Our categories'}
          </h2>
          <p className="text-sm text-[var(--color-slate-500)] mt-0.5">
            {fr ? 'Trouvez rapidement ce que vous cherchez' : 'Find what you need quickly'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
        {categories.map((cat) => {
          const displayName = lang === 'en' ? (CATEGORY_SLUG_EN[cat.slug] ?? cat.name) : cat.name
          return (
            <Link
              key={cat.id}
            ...
            >
              <div className="relative h-14 w-14 rounded-full overflow-hidden bg-[var(--color-slate-100)]">
                {cat.image_url ? (
                  <Image src={cat.image_url} alt={displayName} fill className="object-cover" sizes="56px" />
                ) : (
