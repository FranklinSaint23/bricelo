...
export function CamPayButton({ orderIds, amount }: Props) {
  ...
  async function handlePay() {
    ...
    if (!cleanPhone || cleanPhone.length < 9) {
      setError('Veuillez saisir un numéro Orange Money ou MTN Mobile Money valide.')
      return
    }
      ...
      {!waitingPin ? (
        <div className="flex flex-col gap-3 bg-[var(--color-slate-50)] p-4 rounded-xl border border-[var(--color-slate-200)]">
          <label className="text-xs font-bold text-[var(--color-navy-900)] flex items-center gap-1.5">
            ...
            <span>Numéro Mobile Money (Orange Money / MTN MoMo) *</span>
          </label>
          <div className="relative">
            <Input
              ...
              placeholder="Ex: 699000000 ou 677000000"
              required
          ...
          <p className="text-[11px] text-[var(--color-slate-500)] italic">
            Une demande d'autorisation (Push USSD) sera immédiatement envoyée sur votre mobile.
          </p>
